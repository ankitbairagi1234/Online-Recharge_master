...
class _LoginPageState extends State<Login> with TickerProviderStateMixin {
  ...
  getLoginUser()async{
    ...
    var request = http.Request('POST', Uri.parse('${baseUrl}dboy_login.php'));
    request.body = json.encode({
      "mobile": mobileController.text,
      "password": passwordController.text
    });
    print("ok working ${request.body} and ${baseUrl}dboy_login.php");
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    print("checking result here ${response.statusCode}");
    if (response.statusCode == 200) {
      var finalResult = await response.stream.bytesToString();
      final jsonResponseb = json.decode(finalResult);
      String userid = jsonResponseb['dboydata']['id'].toString();
      String username = jsonResponseb['dboydata']['name'].toString();
      String useremail = jsonResponseb['dboydata']['email'].toString();
      String usermobile = jsonResponseb['dboydata']['mobile'].toString();
      print("final response here ${jsonResponseb} anddddd ${jsonResponseb['dboydata']['id']}");
      ...
      CUR_USERID = userid;
             CUR_USERNAME = username;
              saveUserDetail(userid, username, useremail, usermobile);
